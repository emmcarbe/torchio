...
3. **Assignment begins with classes; exact processing is explicit.** TEI contains more than 500 elements, but organizes them into a few dozen model classes. The map is generated from the official [`p5subset`](https://tei-c.org/Vault/P5/current/xml/tei/odd/p5subset.xml) (P5 4.12.0: 588 elements, 127 model classes, 86 attribute classes, and 35 datatypes), while full coverage is verified through an automated test that can be rerun with every TEI release.
...
Resolution follows one auditable cascade: a matching processing model in the edition ODD; otherwise a conservative Torchio contract associated with the exact TEI element or one of its TEI All model classes; otherwise lossless structural rendering. TEI All is the source of vocabulary and class membership, not a normative visualization profile. The fallback contract is therefore an explicit Torchio decision, marked as such in the output, and can never override the edition ODD.
Development should not, however, be driven by examples, because examples are test cases, not the specification.
4. **The ODD decides, not the tool.** The edition’s ODD customization constitutes its configuration: it defines the modules included, the elements excluded, custom elements, class membership, and explicit processing rules. A custom element declared `memberOf` a TEI class inherits its handling family and page section; full visual behaviour is not inferred from membership alone. Torchio executes a documented web subset of the TEI Processing Model and reports constructs it cannot yet execute. Conformance operates on two levels. The syntactic level can be checked mechanically; the semantic level, by contrast, requires judgement, since an extension is legitimate only when it concerns phenomena not covered by P5. The tool must therefore flag cases that require editorial review, without claiming to resolve them automatically.
5. **The edition is the model.** The engine transforms the TEI into a data model — documents, entity registries, witnesses, hands, layers, and typed apparatuses — and every page and every export, whether XML, CSV, or JSON, is a projection of that model. The same XML must deterministically produce the same model. The model’s written documentation is an acknowledged debt: at present, the model is defined by the code and the exports rather than by a standalone document. How a mention resolves to an entity is part of that model, and is stated here rather than left to the code: a mention joins a register entry by its pointer (`@ref="#id"`), by a shared authority URI (VIAF, Wikidata, GeoNames), or by canonical-name key (`@key`) matched — normalising spacing and case — against the name and the declared key of what the register already holds, so that a register keyed by identifier and its mentions written by canonical name denote one entity, with one index entry and one occurrence count, and not two. An identifier that matches nothing already declared becomes an entity of its own, keyed by that value; identity is attested by the markup, and never inferred from resemblance beyond an exact canonical match (C129, reported by Elena Pierazzo at Editopia, 2 September 2026).
...
| Maurizio Lana                             | 3 August   | asked for a printed form of the edition: the text above, the philological notes below it, and the front matter of a book. He also confirmed the need the engine addresses, a text annotated in TEI being unreadable to anyone who does not read TEI                                                                              | ROADMAP, *The printed edition*, with the three routes considered                |
...
facsimiles are not yet rendered; conservative class contracts exist only for
behaviours that can be assigned without guessing an editorial interpretation;
